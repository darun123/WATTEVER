...
import { auth, googleProvider, appleProvider, signInWithPopup, signInWithPhoneNumber, setupRecaptcha } from '../firebaseConfig'
...
export default function UserModal({ onConfirm, onClose }) {
  ...
  useEffect(() => {
    if (mode === 'login' && !window.recaptchaVerifier) {
  ...
  const handleGuestContinue = async () => {
    ...
    setLoading(false)
    onConfirm({ name: form.name, phone: form.phone, email: form.email, token: null })
  }
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', marginBottom: '20px' }}>
              ...
              {!otpSent ? (
                <>
                  ...
                  <button className="social-btn" onClick={() => handleSocialLogin(appleProvider)}>
                ...
                <>
                  <p style={{ color: 'var(--accent)', fontSize: '14px', marginBottom: '10px' }}>OTP sent to {form.phone}!</p>
                  <div>
                    <label style={{ display: 'block', fontSize: '12px', color: 'var(--text-secondary)', marginBottom: '6px', fontWeight: '500' }}>Enter OTP</label>
                    <input
                      className="input-field"
                  ...
                  <button className="btn-ghost" onClick={() => { setOtpSent(false); setOtp(''); }}>
                    Change Number
                  </button>
                </>
              )}
